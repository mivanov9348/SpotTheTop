...
const PostEditor = ({ onPostCreated, quoteContent, clearQuote }) => {
    ...
    useEffect(() => {
        if (quoteContent) {
            ...
            setTimeout(() => { if (quillRef.current) quillRef.current.focus(); }, 100);
        }
    }, [quoteContent]);
...
const PostCard = ({ post, onActionTriggered, onQuote, currentUserEmail, currentUserRoles }) => {
    ...
    const canDelete = (authorEmail) => {
        ...
        return isAdmin || isAuthor || post.canDelete; // Използваме и бекенд флага, ако го има
    };
    ...
    return (
        <div className="card border-0 rounded-4 shadow-lg post-card mb-4" style={{ backgroundColor: '#1e293b', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="card-body p-4">
                ...
                <div className="mb-4 fs-5 rich-text-content" style={{ color: '#e2e8f0', lineHeight: '1.6' }} dangerouslySetInnerHTML={{ __html: post.content }} />
